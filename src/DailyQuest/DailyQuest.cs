...
public class DailyQuest {
  ...
  public static string TodayDailyQuestFilePath {
    get {
      string dirPath =
          Path.Combine(_programDirectoryPath, DataBaseDirectoryName);

      return Path.Combine(Directory.CreateDirectory(dirPath).FullName,
                          TodayDailyQuestFileName);
    }
  }
  ...
  /// 프로그램 실행파일의 위치
  /// </summary>
  private static string _programDirectoryPath =>
  ...
  /// 만약 데이터의 <see cref="RefreshTime"/>이 현재 시간보다 하루 전이라면
  /// 데이터를 초기화하고 새로운 데이터를 반환합니다.<para/> 만약 파일이
  /// 존재하지 않으면 파일을 새로 생성하고 초기화된 데이터를 반환합니다.
  /// </returns>
  ...
  /// <summary>
  /// 오늘의 일일퀘스트 파일에 매개변수로 받은 데이터를 작성하고 <see
  /// cref="RefreshTime"/>을 현재 시간으로 변경합니다.
  /// </summary>
  ...
  /// <summary>
  /// 기본 일일퀘스트 파일에서 목록을 가져옵니다. 파일이 없다면 파일을
  /// 생성합니다.
  /// </summary>
  /// <returns>
  ...
  public static DailyQuestItem GetDefaultDailyQuest() {
    DailyQuestItem dailyQuest =
        new DailyQuestItem { RefreshTime = DateTime.Now,
    ...
    try {
      ...
      if (!string.IsNullOrWhiteSpace(input)) {
        if (bool.TryParse(input, out bool useEmoji)) {
          ...
          dailyQuest.Quests.Add(
              new DailyQuestItem.Quest { QuestDescription = input });
        }
      }
